"""S2 的"首 token 延迟"基准：同一个问题跑 N 次，量出 p50/p95。
...
def bench_non_streaming(llm: LLM, n: int) -> list[float]:
    latencies = []
    for i in range(n):
        ...
        print(f"  [非流式 {i + 1}/{n}] {latencies[-1]:.0f}ms")
    return latencies


async def bench_streaming(llm: LLM, n: int) -> list[float]:
    latencies = []
    for i in range(n):
        messages = [{"role": "user", "content": QUESTION}]
        start = time.monotonic()
        ...
        print(f"  [流式首token {i + 1}/{n}] {latencies[-1]:.0f}ms")
...
def main() -> None:
    ...
    non_streaming = bench_non_streaming(llm, N)

    print("\n测流式首 token（S2 模式）...")
    streaming = asyncio.run(bench_streaming(llm, N))
